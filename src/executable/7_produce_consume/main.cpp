...
struct producer
{
    ...
    )
    {
        if (consumeContract_)
            return false;
        consumeContract_ = wct.create_contract([this, c = std::forward<decltype(consume)>(consume)](auto & wc){c(pipe_.pop()); if (!pipe_.empty()) wc.schedule();},
...
struct consumer 
{
    bool join(producer_concept auto && producer, bcpp::work_contract_tree & wct)
    {
